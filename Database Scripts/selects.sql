...
JOIN Size s ON s.ID = po.Size_ID where shopping_Cart_ID = 1 ;
...
-- pizza toppings
select t.Topping_Name from Pizza p JOIN PizzaToppings tp ON p.ID = tp.Pizza_ID
JOIN Toppings t ON t.ID = tp.Topping_ID 

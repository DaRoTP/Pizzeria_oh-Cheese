...
import sample.Utilities.HibernateUtil;
import sample.model.client_table;
...
public class Controller {
    ...
    public void display_1(ActionEvent event){
        ...
        try {
            ...
            Query query = session.createQuery("from client_table");
            List users = query.list();


            for(int i = 0; i < users.size(); i++)
                System.out.println("- "+((client_table) users.get(i)).getId()+" = "+((client_table) users.get(i)).getName());
